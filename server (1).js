...
app.post('/leads', async (req, res) => {
  try {
    const { nome, telefone, metodo_pagamento, valor_avista, faixa_mensal } = req.body;

    if (!nome || !telefone || !metodo_pagamento) {
      return res.status(400).json({ erro: 'Campos obrigatórios faltando: nome, telefone, metodo_pagamento.' });
    }
    ...
    console.log(`📥 Novo lead: ${nome} | ${telefone} | ${metodo_pagamento}`);
